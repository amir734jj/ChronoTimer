...
public class testImportExport {
	...
	public void testXML() {
		...
		boolean[] enn = Arrays.copyOf(Channel.channelEnabled, 12);
		boolean[] conn = Arrays.copyOf(Channel.channelConnected, 12);

		ImportExport.ExportSystemToXML();
		ImportExport.ImportSystemFromXML();

		assertTrue(RaceCollection.array.get(0).deepEqualityTest(race1));
		assertTrue(RaceCollection.array.get(1).deepEqualityTest(race2));

		for (int i = 0; i <= 11; i++) {
			assertEquals(conn[i], Channel.channelConnected[i]);
			assertEquals(enn[i], Channel.channelEnabled[i]);
		}

		assertTrue(RaceCollection.CurrentRace.deepEqualityTest(race2));
	}
}
